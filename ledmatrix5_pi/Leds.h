...
class Leds {
  public:
    ...
    void display(int duration = 0); // displays the current set of data for <duration> milliseconds
    uint8_t* getDumpMap();
    // rotates the matrix by
    // pov = 0 => 0   degrees, pov = 1 => 90  degrees,
    // pov = 2 => 180 degrees, pov = 3 => 270 degrees
    void setPointOfView(int pov);
  ...
  private:
    ...
    typedef uint8_t map_t[LEDNUM];
    uint8_t maps[4][LEDNUM] = { // maps virtual LEDs to the real hardware
    ...
    uint8_t dumpMaps[4][LEDNUM]; // maps virtual LEDs for use in dumpHooks
    void mapLedsToBits();
    void swapKeyValue(uint8_t* dst, uint8_t* src, size_t num);
    void setupPointOfView(map_t* maps, int pov);
};
